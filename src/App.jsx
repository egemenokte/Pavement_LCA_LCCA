...
export default function App() {
  ...
  const calcYearlyBreakdown = (alt, dr, fuelCost, co2PerGal, cpiAdj, deterministic = true) => {
    ...
    for (let y = 1; y <= params.analysisPeriod; y++) {
      ...
      yearly.push({ year: y, treatCostDisc, treatCostUndisc, treatCO2, wzCostDisc, wzCostUndisc, wzCO2,
  ...
  const runAnalysis = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const probResults = alternatives.map((alt, idx) => {
        ...
        return { name: alt.name, color: COLORS[idx % COLORS.length],
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-blue-100 p-3">
      <div className="max-w-6xl mx-auto">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl shadow-lg p-4 mb-4 text-white flex items-center justify-center gap-4">
          <RoadIcon />
          <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => {
                  if (newTreatmentName && !treatments[newTreatmentName]) {
                    setTreatments({...treatments, [newTreatmentName]: {
          ...
          <div className="bg-white rounded-xl shadow-lg p-4">
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart margin={{top: 5, right: 20, left: 10, bottom: 40}}>
                    ...
                    <YAxis label={{value: analysisCategory === 'cost' ? 'Cumulative ($M)' : 'Cumulative (t CO₂e)', angle: -90, position: 'insideLeft', offset: 5}} />
                    <Tooltip formatter={(v) => [v.toFixed(3), analysisCategory === 'cost' ? '$M' : 't CO₂e']} />
                    <Legend verticalAlign="top" height={36} />
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded text-xs">
                  <strong>💡 Note:</strong> Costs are <em>discounted</em> to present value. Emissions are <em>not discounted</em>. This may cause different rankings.
                </div>
              </div>
            )}
            ...
            {results && analysisMode === 'probabilistic' && (
              <div>
                  ...
                  <div>
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <AreaChart margin={{top: 5, right: 20, left: 10, bottom: 40}}>
                        ...
                        <YAxis hide />
                        <Tooltip formatter={(v,n) => [v.toFixed(4), 'Density']} labelFormatter={v => `${v.toFixed(3)} ${getUnit()}`} />
                        <Legend verticalAlign="top" height={36} />
                  ...
                  <div>
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <LineChart margin={{top: 5, right: 20, left: 10, bottom: 40}}>
                        ...
                        <Tooltip formatter={(v) => [(v*100).toFixed(1) + '%', 'Probability']} labelFormatter={v => `${v.toFixed(3)} ${getUnit()}`} />
                        <Legend verticalAlign="top" height={36} />
                        {results.probabilistic.map((r) => {
                          const d = r[getProbKey()];
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded text-xs">
                  <strong>💡 Note:</strong> Costs are <em>discounted</em>. Emissions are <em>not discounted</em>. Rankings may differ.
                </div>
              </div>
            )}
          </div>
        ...
        <p className="text-center text-xs text-gray-500 mt-4">
          For Educational Purposes Only. By <a href="https://egemenokte.com" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">Egemen Okte</a>
        </p>
      </div>
    </div>
  );
